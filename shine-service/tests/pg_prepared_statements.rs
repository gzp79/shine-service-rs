...
async fn test_stored_statements() {
    match env::var("SHINE_TEST_PG_CNS") {
        Ok(cns) => {
            ...
            let p1 = c1.query_one(&stmt.get(&c1).await.unwrap(), &[]).await.unwrap();
            let p2 = c2.query_one(&stmt.get(&c2).await.unwrap(), &[]).await.unwrap();

            // make sure the prepared statements are unique to the connection and the test checks the prepared statements handling
            assert!(c1.query_one(&stmt.get(&c2).await.unwrap(), &[]).await.is_err());
            assert!(c2.query_one(&stmt.get(&c1).await.unwrap(), &[]).await.is_err());
